Public Class GetValues
    ...
    Public Function getValuesFromExcelExpand() As DataSet
        Dim selection_area As Integer
        Dim selection_column As Integer
        Dim selection_row As Integer
        Dim current_columnCount As Integer
        Dim curent_rowCount As Integer
            ...
            If Worksheet IsNot Nothing Then
                ...
                If selection_area >= 1 Then
                    ...
                    For Each singleArea In Excel.Selection.Areas
                        ...
                        selection_row = singleArea.Rows.Count
                        selection_column = singleArea.Columns.Count
                        If selection_column = 1 And selection_row = 1 Then
                            currentArea = Excel.Range(singleArea, singleArea.End(Microsoft.Office.Interop.Excel.XlDirection.xlDown).Resize(, 5))
                        Else
                            currentArea = singleArea
                        End If
                        range = currentArea.cells.value
                        curent_rowCount = currentArea.Rows.Count
                        current_columnCount = currentArea.Columns.Count
                        ' MessageBox.Show(range(1, 1).ToString().Trim())
                        newTable = True
                        For i = 1 To curent_rowCount
                            If range(i, 1) Is Nothing OrElse range(i, 1).ToString().Trim() = "" Then
                                newTable = True
                            Else
                                If newTable = True Then
                                    ...
                                    dt = New DataTable(range(i, 1), Nspace)
                                    dt.Columns.Add("Table")
                                    dt.Columns.Add("Type")
                                    dt.Columns.Add("Constraint")
                                    dt.Columns.Add("Text")
                    ...
                    Next
                Else
                    MsgBox("Please Select at least one Area. ")
                End If
            Else
                MsgBox("Work Sheet not found.")
            End If
